...
def add(request, id, token):
    if not validate_user_session(id, token):
        return JsonResponse({'error':'Please re-login','code':'1'}) #u can put your custom code no.
    ...
    if request.method == "POST":
        ...
        ordr = Order(user=user, product_names =products, total_products=total_pro, 
                     transaction_id=transaction_id,  total_amount = amount) #intentionally ordr named as it is already named
        ordr.save()
